...
# This model is only for H3+ ionisation
...
n0 = 1e20 # m-3
Te = 1000 # K
alpha = 1.15e-7 * (300 / Te) ** 0.65 # cm-3
g = G * M_J / R_J ** 2 # ms-2
print('gravity', g)
k = 1.38e-23 # m2kgs-2K-18
sigma =  6e-22 # m2
C = 1/36.2 # eV-1
I_inf =  6.3e16 / 25 # eVm-2s-1
chi = 0
...
nu_ions = 2.21 * np.pi * (neutrals * 1e-6) * ((m*1e3) /((mi + m)*1e3)) * np.sqrt((0.82 * (ee**2))/mu_in)
nu_en = 4.5e-9 * (neutrals) * (1 - (1.35e-4 * Te)) / Te**0.5
...
electron_p_contr = 0#(1 / ((9.1e-31) * nu_en)) * ((nu_en ** 2) / ((nu_en ** 2) + (Omega_ce ** 2)))
